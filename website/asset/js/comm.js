...
function showComment(page) {
    var url = "asset/getNote.php?page=" + page;
    $.getJSON(url, function(json) {
        var str = "";
        total = json.total[0];
        $.each(json.result, function (index, content) {
            str += '<div class="author_comment">' + '<label id="' + (index + 1) + '" style="display:none">' + content.cid + '</label>'+ "<span class=\"badge badge-info\">#" + (index + 1) + '</span>&nbsp;' +'<a class="name">' + content.userName + '</a>' + "<span class='time'>" + content.postDate + "</span>"+'<span class="area-tool-comment"><a class="btn-quote" onclick="quoteComm($(this))">[回复]</a></span>';
            str += '<div class="content-comment">' + content.contents + "</div>";
            $.each(content.comment2, function (index2, content2) {
                str += '<div class="author_comment last"><a class="name"> &nbsp;' + content2.userName + '</a>' + "<span class='time'>" + content2.postDate + "</span>"  + '<span class="area-tool-comment"><a class="btn-quote" onclick="quoteComm($(this))">[回复]</a></span>'+'<div class="content-comment">' + content2.contents + "</div></div>";
            });
            str += "</div>";
        });
...
function sendComment(param){
    ...
    var url = "asset/getNote.php?userName=" + nameMsg + "&contents=" + textMsg + "&cid=" + posMsg;
    $.getJSON(url);
    //alert(posMsg);
    showComment(0);
}
