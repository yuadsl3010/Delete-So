...
def get_search_results(request):
    ...
    try:
        if request.method != 'POST':
            ...
            #获取用户输入数据
            search = request.POST.get('search', '').encode('utf-8')
            ...
            if len(search) == 0:
                ...
                #获取“带带我”的数据
                if search == '%E5%B8%A6%E5%B8%A6%E6%88%91':
                    ...
                    #构造回显的json
                    json_result['total'] = total_searchs
                    json_result['result'] = result_searchs
                #获取ac投稿的数据
                else:
                    if search.find('ac') == 0:
                        search = search[2:]
                    ...
                    print 'comments cost: ' + str(time_b - time_a)
                    result_searchs = []
                    ...
                    print 'info cost: ' + str(time_b - time_a)
                    dict_contents_info = model_to_dict(qs_info)    
                    for qs in qs_searchs:
                        dict_searchs = model_to_dict(qs)
                        dict_searchs.update(dict_contents_info)
